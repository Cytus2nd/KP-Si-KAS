...
$query .= " ORDER BY kas_pramuka.created_at DESC LIMIT ? OFFSET ?";
$params[] = $limit;
$params[] = $offset;
...
if ($tipe_kas != 'semua') {
    $total_query .= " AND kas_pramuka.tipe_kas = ?";
    $total_params[] = $tipe_kas;
}
if (!empty($cari)) {
    $total_query .= " AND (kas_pramuka.keterangan LIKE ? OR kas_pramuka.jumlah LIKE ?)";
    $total_params[] = "%$cari%";
    $total_params[] = "%$cari%";
}
