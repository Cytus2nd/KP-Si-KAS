...
$query .= " ORDER BY kas_pmr.created_at DESC";
...
usort($data_pmr, function ($a, $b) {
    return strtotime($a['created_at']) - strtotime($b['created_at']);
});

foreach ($data_pmr as $pmr) {
    if ($pmr['tipe_kas'] == 'pemasukan') {
        $total_pemasukan += $pmr['jumlah'];
    } else if ($pmr['tipe_kas'] == 'pengeluaran') {
        $total_pengeluaran += $pmr['jumlah'];
    }
}

// Calculate universal totals
$query_universal_pemasukan = "SELECT SUM(jumlah) as total_pemasukan FROM kas_pmr WHERE tipe_kas='pemasukan'";
$query_universal_pengeluaran = "SELECT SUM(jumlah) as total_pengeluaran FROM kas_pmr WHERE tipe_kas='pengeluaran'";
...
$universal_pemasukan_pmr = $data_universal_pemasukan['total_pemasukan'] ?? 0;
...
$universal_pengeluaran_pmr = $data_universal_pengeluaran['total_pengeluaran'] ?? 0;

$universal_total_kas_pmr = $universal_pemasukan_pmr - $universal_pengeluaran_pmr;

// Output the results (example)
$pdf->SetFont('Times', 'B', 12);
$pdf->Cell(0, 10, 'LAPORAN KAS PMR', 0, 1, 'C');
...
$pdf->Cell(30, 10, 'Rp ' . number_format($universal_total_kas_pmr, 0, ',', '.'), 0, 1, 'R');
$pdf->Ln(4);
...
foreach ($data_pmr as $pmr) {
    $pdf->Cell(10, 10, $no++, 1);
    $pdf->Cell(40, 10, date('d/m/Y H:i', strtotime($pmr['created_at'])), 1);
    ...
    $pdf->Cell(40, 10, htmlspecialchars($pmr['tipe_kas']), 1);

    // Reset to default color (black) for the rest of the cells
    $pdf->SetTextColor(0, 0, 0);
    $pdf->Cell(40, 10, 'Rp ' . number_format($pmr['jumlah'], 0, ',', '.'), 1);
    $pdf->Cell(60, 10, htmlspecialchars($pmr['keterangan']), 1);
    $pdf->Ln();
}
$pdf->Ln(4);
...
// Output the PDF
$pdf->Output('D', 'Laporan Kas PMR Bulan ' . $bulan . ' Tahun ' . $tahun . '.pdf');
